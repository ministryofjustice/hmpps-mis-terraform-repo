...
resource "aws_ssm_parameter" "nextcloud_db_password" {
  ...
  type        = "SecureString"
  value       = "${substr(sha256(bcrypt(random_string.password.result)),0,var.password_length)}${random_string.nextcloud_special.result}"

  tags = "${merge(local.tags, map("Name", "${local.environment_identifier}-db-password"))}"
...
resource "aws_ssm_parameter" "nextcloud_db_user" {
  ...
  tags        = "${merge(local.tags, map("Name", "${local.environment_identifier}-db-user"))}"
}
...
resource "random_string" "nextcloud_special" {
  ...
  resource "aws_ssm_parameter" "nextcloud_ssm_password" {
    ...
    tags = "${merge(local.tags, map("Name", "${local.environment_identifier}-admin-password"))}"
  ...
  resource "aws_ssm_parameter" "nextcloud_ssm_user" {
    ...
    tags        = "${merge(local.tags, map("Name", "${local.environment_identifier}-admin-user"))}"
  }
