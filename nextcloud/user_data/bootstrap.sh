...
yum -y install httpd php php-cli php-mysqlnd php-zip php-devel php-gd php-mcrypt php-mbstring php-curl php-xml php-pear php-bcmath php-json php-pdo php-pecl-apcu php-pecl-apcu-devel php-intl php71-php-pecl-imagick ;
...
sudo -u apache php occ maintenance:install --database "mysql" \
...
sudo -u apache php occ config:system:set trusted_domains 0 --value=nextcloud.$EXTERNAL_DOMAIN ;
sudo -u apache php occ config:system:set overwritehost --value=nextcloud.$EXTERNAL_DOMAIN ;
sudo -u apache php occ config:system:set overwriteprotocol --value=https ;
sudo -u apache php occ config:system:set overwrite.cli.url --value=https://$EXTERNAL_DOMAIN ;
