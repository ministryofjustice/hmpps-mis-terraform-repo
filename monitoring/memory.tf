...
resource "aws_cloudwatch_metric_alarm" "bws_instance-memory-alert" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__alert__BWS__${data.terraform_remote_state.ec2-ndl-bws.outputs.bws_instance_ids[count.index]}"
  ...
  statistic           = "Average"
  threshold           = "85"
  alarm_description   = "Memory Utilization  is averaging 85% for ${data.terraform_remote_state.ec2-ndl-bws.outputs.bws_primary_dns_ext[count.index]}. Please contact the MIS Team or the MIS AWS Support Contact."
...
resource "aws_cloudwatch_metric_alarm" "bcs_instance-memory-alert" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__alert__BCS__${data.terraform_remote_state.ec2-ndl-bcs.outputs.bcs_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "85"
  alarm_description   = "Memory Utilization  is averaging 85% for ${data.terraform_remote_state.ec2-ndl-bcs.outputs.bcs_primary_dns_ext[count.index]}. Please contact the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
...
resource "aws_cloudwatch_metric_alarm" "bps_instance-memory-alert" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__alert__BPS__${data.terraform_remote_state.ec2-ndl-bps.outputs.bps_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "85"
  alarm_description   = "Memory Utilization  is averaging 85% for ${data.terraform_remote_state.ec2-ndl-bps.outputs.bps_primary_dns_ext[count.index]}. Please contact the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
...
resource "aws_cloudwatch_metric_alarm" "dis_instance-memory-alert" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__alert__DIS__${data.terraform_remote_state.ec2-ndl-dis.outputs.dis_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "85"
  alarm_description   = "Memory Utilization  is averaging 85% for ${data.terraform_remote_state.ec2-ndl-dis.outputs.dis_primary_dns_ext[count.index]}. Please contact the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
...
resource "aws_cloudwatch_metric_alarm" "bfs_instance-memory-alert" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__alert__BFS__${data.terraform_remote_state.ec2-ndl-bfs.outputs.bfs_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "85"
  alarm_description   = "Memory Utilization  is averaging 85% for ${data.terraform_remote_state.ec2-ndl-bfs.outputs.bfs_primary_dns_ext[count.index]}. Please contact the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
...
resource "aws_cloudwatch_metric_alarm" "bws_instance-memory-severe" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__severe__BWS__${data.terraform_remote_state.ec2-ndl-bws.outputs.bws_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "88"
  alarm_description   = "Memory Utilization  is averaging 88% for ${data.terraform_remote_state.ec2-ndl-bws.outputs.bws_primary_dns_ext[count.index]}. Please contact the MIS Team or the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
...
resource "aws_cloudwatch_metric_alarm" "bcs_instance-memory-severe" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__severe__BCS__${data.terraform_remote_state.ec2-ndl-bcs.outputs.bcs_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "88"
  alarm_description   = "Memory Utilization  is averaging 88% for ${data.terraform_remote_state.ec2-ndl-bcs.outputs.bcs_primary_dns_ext[count.index]}. Please contact the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
...
resource "aws_cloudwatch_metric_alarm" "bps_instance-memory-severe" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__severe__BPS__${data.terraform_remote_state.ec2-ndl-bps.outputs.bps_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "88"
  alarm_description   = "Memory Utilization  is averaging 88% for ${data.terraform_remote_state.ec2-ndl-bps.outputs.bps_primary_dns_ext[count.index]}. Please contact the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
...
resource "aws_cloudwatch_metric_alarm" "dis_instance-memory-severe" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__severe__DIS__${data.terraform_remote_state.ec2-ndl-dis.outputs.dis_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "88"
  alarm_description   = "Memory Utilization  is averaging 88% for ${data.terraform_remote_state.ec2-ndl-dis.outputs.dis_primary_dns_ext[count.index]}. Please contact the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
...
resource "aws_cloudwatch_metric_alarm" "bfs_instance-memory-severe" {
  ...
  alarm_name          = "${local.environment_name}__Memory-Utilization__severe__BFS__${data.terraform_remote_state.ec2-ndl-bfs.outputs.bfs_instance_ids[count.index]}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "MemoryUtilization"
  namespace           = "CWAgent"
  ...
  statistic           = "Average"
  threshold           = "88"
  alarm_description   = "Memory Utilization  is averaging 88% for ${data.terraform_remote_state.ec2-ndl-bfs.outputs.bfs_primary_dns_ext[count.index]}. Please contact the MIS AWS Support Contact."
  alarm_actions       = [aws_sns_topic.alarm_notification.arn]
  ok_actions          = [aws_sns_topic.alarm_notification.arn]
