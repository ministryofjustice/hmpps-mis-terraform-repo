...
locals {
  ...
  server_dns_names = ["*.${data.terraform_remote_state.common.internal_domain}"]

  server_subject = {
    common_name  = "${data.terraform_remote_state.common.internal_domain}"
    organization = "${data.terraform_remote_state.common.common_name}-ldap"
  }
}
