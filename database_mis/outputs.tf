...
locals {
  ...
  public_fqdn = substr(
    ...
    0,
    local.public_fqdn_length,
  )

  # db_size_delius_core attribute may not be set in all envs, if not we default to two
  high_availability_count = var.database_high_availability_count["mis"]
  empty                   = ""
  db1                     = "mis-db-1.${local.public_fqdn}"
  db2                     = "mis-db-2.${local.public_fqdn}"
  db3                     = "mis-db-3.${local.public_fqdn}"
