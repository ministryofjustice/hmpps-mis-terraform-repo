...
DESTINATION_LOCATION_ARN=$(aws datasync create-location-fsx-windows --fsx-filesystem-arn "${FSX_ARN}" --security-group-arns "${FSX_SG_ARN}" --user "${FSX_USER}" --password "${FSX_PASS}" --subdirectory "share/DFInterventions"   --domain "${FSX_DOMAIN}" --tags "Key=Name,Value=$NAME" --region "${REGION}" | jq -r .LocationArn) || exit $?
